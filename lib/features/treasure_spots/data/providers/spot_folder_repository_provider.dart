...
final spotFolderRepositoryProvider = Provider<SpotFolderRepository>((ref) {
  return SpotFolderRepositoryImpl(db: DatabaseService.instance, onChanged: () => ref.read(treasureSpotsRefreshProvider.notifier).bump());
});

final spotFolderListProvider = FutureProvider<List<SpotFolder>>((ref) async {
  ref.watch(treasureSpotsRefreshProvider);
  return ref.watch(spotFolderRepositoryProvider).listAll();
});

final spotFolderDetailProvider = FutureProvider.autoDispose.family<SpotFolder?, String>((ref, id) async {
  ref.watch(treasureSpotsRefreshProvider);
  return ref.watch(spotFolderRepositoryProvider).getById(id);
});
...
class SpotFolderListNotifier extends AsyncNotifier<void> {
  ...
  Future<SpotFolder> createFolder({required String name, IconData? icon, Color? color, int? sortOrder, String? id}) async {
    SpotFolder? created;
    state = const AsyncValue.loading();
    state = await AsyncValue.guard(() async {
      final folder = SpotFolder(id: id ?? '', name: name, iconCode: icon?.codePoint ?? Icons.folder.codePoint, colorValue: (color ?? Colors.blue).value, sortOrder: sortOrder ?? 0);
      created = await _repo.create(folder);
    });
    _bump();
    if (state.hasError) Error.throwWithStackTrace(state.error!, state.stackTrace!);
    if (created == null) throw StateError('文件夹创建失败');
    return created!;
  }
  ...
  Future<SpotFolder> updateFolder(SpotFolder folder) async {
    SpotFolder? updated;
    state = const AsyncValue.loading();
    state = await AsyncValue.guard(() async { updated = await _repo.update(folder); });
    _bump();
    if (state.hasError) Error.throwWithStackTrace(state.error!, state.stackTrace!);
    if (updated == null) throw StateError('文件夹更新失败');
    return updated!;
  }
...
final spotFolderListNotifierProvider = AsyncNotifierProvider<SpotFolderListNotifier, void>(SpotFolderListNotifier.new);
