...
class TreasureSpotRepositoryImpl implements TreasureSpotRepository {
  ...
  TreasureSpotRepositoryImpl({required this.db, this.onChanged});
  ...
  Future<TreasureSpot> create(TreasureSpot spot) async {
    ...
    await database.insert('treasure_spots', TreasureSpotDbAdapter.toRow(effectiveSpot));
    onChanged?.call();
  ...
  Future<TreasureSpot> update(TreasureSpot spot) async {
    ...
    await database.update('treasure_spots', TreasureSpotDbAdapter.toRow(effectiveSpot), where: 'id = ?', whereArgs: [spot.id]);
    onChanged?.call();
    return effectiveSpot;
  }
  ...
  Future<void> delete(String id) async {
    final database = await db.database;
    await database.delete('treasure_spots', where: 'id = ?', whereArgs: [id]);
    onChanged?.call();
  }
  ...
  Future<TreasureSpot?> getById(String id) async {
    final database = await db.database;
    final rows = await database.query('treasure_spots', where: 'id = ?', whereArgs: [id], limit: 1);
  ...
  Future<List<TreasureSpot>> listAll() async {
    final database = await db.database;
    final rows = await database.query('treasure_spots', orderBy: 'updated_at DESC, created_at DESC');
  ...
  Future<List<TreasureSpot>> listByFolder(String? folderId) async {
    ...
    if (folderId == null) {
      rows = await database.query('treasure_spots', where: 'folder_id IS NULL', orderBy: 'updated_at DESC, created_at DESC');
    } else {
      rows = await database.query('treasure_spots', where: 'folder_id = ?', whereArgs: [folderId], orderBy: 'updated_at DESC, created_at DESC');
    }
    return rows.map(TreasureSpotDbAdapter.fromRow).toList();
  }
  ...
  Future<List<TreasureSpot>> searchByName(String keyword) async {
    ...
    final like = '%$kw%';
    final rows = await database.query('treasure_spots', where: 'name LIKE ? OR address LIKE ? OR note LIKE ?', whereArgs: [like, like, like], orderBy: 'updated_at DESC');
    return rows.map(TreasureSpotDbAdapter.fromRow).toList();
  }
}
