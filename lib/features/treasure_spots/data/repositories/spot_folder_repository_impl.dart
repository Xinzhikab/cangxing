...
class SpotFolderRepositoryImpl implements SpotFolderRepository {
  ...
  SpotFolderRepositoryImpl({required this.db, this.onChanged});

  @override
  Future<SpotFolder> create(SpotFolder folder) async {
    final database = await db.database;
    final effectiveId = folder.id.isEmpty ? _uuid.v4() : folder.id;
    final effectiveFolder = folder.id.isEmpty ? folder.copyWith(id: effectiveId) : folder;
    await database.insert('spot_folders', SpotFolderDbAdapter.toRow(effectiveFolder));
    onChanged?.call();
  ...
  Future<SpotFolder> update(SpotFolder folder) async {
    final database = await db.database;
    await database.update('spot_folders', SpotFolderDbAdapter.toRow(folder), where: 'id = ?', whereArgs: [folder.id]);
    onChanged?.call();
    return folder;
  }
  ...
  Future<void> delete(String id) async {
    ...
    await database.transaction((txn) async {
      await txn.update('treasure_spots', {'folder_id': null}, where: 'folder_id = ?', whereArgs: [id]);
      await txn.delete('spot_folders', where: 'id = ?', whereArgs: [id]);
    });
    onChanged?.call();
  }
  ...
  Future<List<SpotFolder>> listAll() async {
    final database = await db.database;
    final rows = await database.query('spot_folders', orderBy: 'sort_order ASC, created_at ASC');
  ...
  Future<SpotFolder?> getById(String id) async {
    final database = await db.database;
    final rows = await database.query('spot_folders', where: 'id = ?', whereArgs: [id], limit: 1);
