      ...
      body: foldersAsync.when(
        ...
        data: (folders) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (_, i) {
              final f = folders[i];
              // folderSpotCountProvider 是 FutureProvider → AsyncValue<int>
...
class _FolderTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return ListTile(
      leading: CircleAvatar(
        ...
        child: Icon(folder.icon, color: folder.color), // 用新增的 icon getter
      ),
  ...
  Future<void> _confirmDelete(BuildContext context, WidgetRef ref) async {
    if (count > 0) {
      final choice = await showDialog<int>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () => Navigator.pop(ctx, 1), // 仅删文件夹
              child: const Text('仅删文件夹'),
            ),
            FilledButton(
              style: FilledButton.styleFrom(
                backgroundColor: Theme.of(ctx).colorScheme.error,
              ),
              onPressed: () => Navigator.pop(ctx, 2), // 全部删除
      ...
      if (choice == 2) {
        // 删除该文件夹下所有地点：先从 repo 列出来，再逐个删除
        final repo = ref.read(spotRepositoryProvider);
        final spots = await repo.listByFolder(folder.id);
        for (final s in spots) {
          await spotNotifier.deleteSpot(s.id);
        }
      }
      // （choice=1 时，Repository.delete 内部会把下属 spot 的 folder_id 置空）
      // 删除文件夹本身
      ...
      final ok = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          title: const Text('删除文件夹'),
          content: Text('确定删除空文件夹「${folder.name}」？'),
          actions: [
            TextButton(
      ...
      if (ok == true) {
        await ref
            .read(spotFolderListNotifierProvider.notifier)
            ...
            children: [
              ...
              SizedBox(
                ...
                child: GridView.count(
                  ...
                  children: FolderIconPresets.icons.map((i) {
                    final selected = i.codePoint == icon.codePoint;
                    return InkResponse(
                      ...
                      child: Container(
                        ...
                        child: Icon(i, color: selected ? color : null),
                      ),
                    );
                  }).toList(),
                ),
              ...
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: FolderColorPresets.colors.map((c) {
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              final notifier =
                  ref.read(spotFolderListNotifierProvider.notifier);
              if (isEdit) {
